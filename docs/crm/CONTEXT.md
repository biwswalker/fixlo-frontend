...
- **crm_agent_profile** — companion row keyed by Fixlo user id **× project**. Holds
  ...
  `crm_role` governs CRM. The axes are conceptually separate ([[ADR 0001]]), but
  `crm_role` is **derived from the Fixlo role** (owner/admin → supervisor, staff →
  junior, viewer/none → no access) rather than sourced independently ([[ADR 0006]]).
  The column persists for attribution, not as the authorization source.
