...
**ความหมาย**: ผลรวม `amount` ของรายการฝากที่ scraper บันทึกว่า "สำเร็จ" ในช่วงนั้น. กรอง `project_id integer FK` (migration 041) — ไม่ใช่ `web_acc ILIKE` อีกต่อไป.
...
WHERE db.project_id = $projectIntId
