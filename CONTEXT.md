...
- **Transaction date timezone** — ทุก query ที่ filter ด้วยวันที่ต้อง cast ด้วย `(transfer_at AT TIME ZONE 'UTC')::date` (ไม่ใช่ `transfer_at::date`) เพราะ DB session timezone = `Asia/Bangkok` แต่ `transfer_at` เก็บเป็น UTC timestamp — cast โดยตรงจะ shift วันผิด (17:00 UTC = 00:00 Bangkok วันถัดไป).
