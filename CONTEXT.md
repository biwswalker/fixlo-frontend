...
- **Reconciliation day-only** — หน้า reconciliation ใช้ period=day เท่านั้น (ตัด week/month/year ออก). Period selector **ซ่อน** ไม่ใช่ disabled — day-only โดย construction. Pending match banner filter ด้วย `(transfer_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok')::date = targetDate` (ไม่ใช่ backlog ทั้งหมด). Banner link ไป match page พร้อม `?transferDate=YYYY-MM-DD` pre-filled.
