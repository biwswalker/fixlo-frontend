...
- **transfer_at timezone invariant** — `transactions.transfer_at` เก็บเป็น UTC plain timestamp. query ที่ filter ตามวัน Bangkok ต้องใช้ `(transfer_at + INTERVAL '7 hours')::date` เสมอ (ไม่ใช่ `transfer_at::date`). pg-node OID 1114 parser ใน `src/lib/db.ts` configure parse-as-UTC ป้องกัน double-shift ใน Bangkok TZ process. Worker (fixlo-spectre) ใช้ `targetDate` (Discord upload date) เป็น date anchor ไม่ใช่ AI-extracted date เพื่อป้องกัน OCR year error จาก TrueMoney BE year format.
...
- **Transaction date timezone** — `transfer_at` เก็บเป็น Bangkok naive timestamp (`timestamp without time zone`) — DB session timezone = `Asia/Bangkok` — ดังนั้น `transfer_at::date` ให้วันที่ Bangkok ถูกต้องเสมอ **อย่าใช้** `(transfer_at AT TIME ZONE 'UTC')::date` เพราะ interpret naive value เป็น UTC แล้ว session Bangkok บวก +7 ทำให้ transaction ช่วง 17:00–23:59 shift ไปวันถัดไป (bug ที่พบ: reconciliation นับได้ 151 แทนที่จะเป็น 310).
...
- **Unmatched slip/balance banner** — Banner แจ้งเตือน ที่แสดง count ของ unmatched slips + unmatched daily_balances สำหรับ targetDate. Filter: `transactions.transfer_at::date = targetDate AND matching_status NOT IN ('AUTO_MAPPED','MANUAL_MAPPED')` + `daily_balances.date = targetDate AND matching_status = 'UNMATCHED'`. Link ไป match page พร้อม `?transferDate=targetDate` pre-filled. แสดงเฉพาะตอน count > 0.
