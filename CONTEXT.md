...
- **created_at timezone convention** — ⚠️ type `timestamp without time zone` ไม่บอกว่า UTC หรือ BKK — ต้องรู้ per-column. ดู [ADR 0015](docs/adr/0015-created-at-utc-standardization.md) (migration 044, applied 2026-06-02).
  - **app-owned = UTC-naive** (ตรงกับ [[transfer_at timezone invariant]]): `raw_uploads, transactions, daily_balances, project_accounts, projects`. default = `(now() AT TIME ZONE 'UTC')`. (ก่อน migration 044 เป็น Bangkok-naive จาก `CURRENT_TIMESTAMP` + session TZ — backfill −7h แล้ว)
  - **report_\* = ยัง Bangkok-naive** (เขียนโดย fixlo-scraper repo 3, ไม่ migrate — inconsistency ที่ยอมรับ)
  - `manual_transactions.created_at` + `transaction_types.created_at` = `timestamptz` (unambiguous)
  - frontend `src/lib/db.ts` OID 1114 parser = parse-as-UTC → อ่าน app-owned ถูก, แต่ `report_*` (Bangkok-naive) จะเพี้ยน +7h ถ้าโชว์
  - Spectre `resolveBalanceDate` (issue #107): `EXTRACT(EPOCH FROM created_at AT TIME ZONE 'UTC')*1000` → instant จริง แล้ว +7h เป็น BKK ก่อน 06:00 business-day cutoff
  - filter ตามวัน BKK บน UTC column: `(col AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok')::date` (ห้าม `col::date`)
- **Matching** — process จับคู่ slip/daily_balance กับ project_account. ใช้กับ [[transactions]] (slip→account) และ [[daily_balances]] (balance snapshot→account). Daily balance matcher v2 (ดู [ADR 0005](docs/adr/0005-balance-matcher-v2.md) + [ADR 0014](docs/adr/0014-balance-matcher-two-phase-p0.md)): **two-phase** — Phase 1 P0 account_number (exact 100 / suffix 90) ประเมินทุก master ก่อน, P0 hit → return ทันที ไม่รัน P1; >1 P0 hit → PENDING_REVIEW (ambiguous). Phase 2 P1 = name+alias substring/token-fuzzy (Levenshtein ≤ 1 per token, min token length 4, normalize honorific ก่อน), P2 = platform/bank_code tiebreaker (+10 bonus). Threshold ≥85 AUTO, ≥50 PENDING. ถ้าไม่ unique → PENDING_REVIEW → admin match manual หรือ reject.
...
- **ยอดรับ** (per-account inflow) — ยอดรับเงินจริงจาก **player** ของ master account หนึ่งบัญชี สำหรับวันที่เลือก D: `ยอดรับ = (balance_D − balance_(D-1)) + effectiveOutflow_D − parking_in_D` โดย `parking_in_D` = Σ [[Gateway parking withdrawal]] (Approved) ที่เข้า account นั้นในวัน D ([ADR 0018](docs/adr/0018-parking-carve-out-reconciliation.md) — carve เงินโยกพักออก ไม่ให้ double-count กับ Apay deposit). ยอดรับ ติดลบได้ (= balance ไม่ตรง parking ที่บันทึก, flag ให้ admin ดู — ไม่ floor). ใช้ **strict date match** — ถ้าไม่มี balance ตรงวันใดวันหนึ่ง แสดงข้อความแทนตัวเลข: "ไม่มียอดคงเหลือวันที่เลือก" / "ไม่มียอดคงเหลือวันก่อนหน้า" / "ไม่มียอดคงเหลือทั้งสองวัน". สอดคล้องกับ **Daily balance inflow formula**. **ข้อยกเว้น Apay** (ADR 0016): แถว Apay ไม่ใช้สูตรนี้ — `ยอดรับ = report_apay_daily.deposit_amount`, `effectiveOutflow = withdrawal_amount` (replace). balance เปิด/ปิด ยังโชว์จาก `daily_balances` แต่ display-only ไม่เข้าสูตร. ไม่มี report row วันนั้น → แถวโชว์ "—/ไม่มีรายงาน" (ไม่ fallback สูตร).
...
- **ยอดเข้าระบบ(สลิป)** — ยอดรับรวมทุกบัญชี คำนวณจากสลิป = SUM ของ **ยอดรับ** ทุก account ที่มีข้อมูล balance ครบทั้งสองวัน (partial sum — บัญชีที่ขาดข้อมูลไม่นับเข้า). แถว Apay contribute `deposit_amount` แทน balance-formula (ADR 0016). แสดงใน KPI card slot 2 ของหน้า reconciliation.
...
- **Reconciliation day-only** — หน้า reconciliation ใช้ period=day เท่านั้น (ตัด week/month/year ออก). Period selector **ซ่อน** ไม่ใช่ disabled — day-only โดย construction. Pending match banner filter ด้วย `(transfer_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok')::date = targetDate` (ไม่ใช่ backlog ทั้งหมด). Banner link ไป match page พร้อม `?date=YYYY-MM-DD` pre-filled.
...
- **`report_apay_daily`** — daily aggregate ยอดฝาก/ถอน/fee จาก Apay gateway (1 row ต่อวันต่อ account ต่อ source). **เข้าสูตรแล้ว** (ADR 0016 supersede ส่วน "cross-check only" ของ ADR 0009): `deposit_amount`/`withdrawal_amount` กลายเป็น source of truth ของแถว Apay ในตาราง account breakdown — replace ค่าที่คำนวณจาก balance. UNIQUE `(date, project_account_id, source)` → ON CONFLICT DO UPDATE. ดู [ADR 0009](docs/adr/0009-apay-gateway-scraper.md) + [ADR 0016](docs/adr/0016-apay-gateway-into-reconciliation-table.md).
...
- **Gateway parking withdrawal** (รายการโยกเงินพัก) — รายการ **per-transaction** ที่ gateway โยกเงินถอนออกมาพัก/เก็บที่ master account ของโปรเจกต์เอง (ไม่ใช่จ่าย player). Scrape จาก apay merchant portal (`linkdeepcode.me`, login `accteam001` = "ACCTEAM" เดียวกับ Apay) หน้า "Merchant Withdrawal list" — ราย transaction มี timestamp, ไม่ใช่ EOD aggregate. **ต่างจาก 2 concept ที่ชื่อคล้าย**: (1) withdrawal slip ([[transactions]]) = จ่าย player ผ่าน Discord; (2) `report_apay_daily.withdrawal_amount` = daily aggregate. เก็บใน table `gateway_parking_withdrawals` (per-transaction grain). ดู [ADR 0017](docs/adr/0017-gateway-parking-withdrawal-capture-first.md).
  - **Parking carve-out** (phase 2, [ADR 0018](docs/adr/0018-parking-carve-out-reconciliation.md)) — parking ที่ Approved เข้า master = เงินที่นับไปแล้วตอนเข้า Apay gateway (`deposit_amount`) แล้ว sweep เข้า bank → ถ้าไม่หักจะ **double-count** ใน ยอดรับ ของ master. สูตร carve: `ยอดรับ_player = (balance_D − balance_(D-1)) + effectiveOutflow − parking_in_D`. ดู [[ยอดรับ]]. parking ที่ match master ไม่เจอ (FK null) = เข้า account นอกระบบ ไม่มี balance snapshot → ไม่ double-count → ไม่ carve (display/audit เท่านั้น).
  - **Idempotency**: `UNIQUE(project_id, order_id)` → `ON CONFLICT DO UPDATE` เพราะ **status เปลี่ยนได้** (Pending→Approved ระหว่างรอบ scrape).
  - `project_id` = จาก scraper config (1 login = 1 project, เหมือน Apay เดิม).
  - **Destination account** = master account ที่เงินเข้าไปพัก. เก็บ **raw text** `account_number` + `account_name` ก่อน (nullable `project_account_id` FK เพิ่มทีหลังตอน reconciliation phase) — ตอน capture ไม่แบก matcher.
  - **Amounts** (เก็บครบ 4 จาก portal): `request_amount` (ยอดขอถอน gross), `refund_amount` (ยอดหัก/คืน), `amount` (**canonical** = สุทธิเข้า master = request − refund), `fee_amount`. P2P% + Bank ทิ้ง (matching quality / redundant).
  - `transfer_at` = portal row datetime (Bangkok local) → แปลง **UTC** ก่อน INSERT ตาม [[transfer_at timezone invariant]] (column `timestamp without time zone` = UTC-naive).
  - **Status** = เก็บ **ทุกค่าเป็น raw text** ไม่ filter ตอน capture (Pending/Approved/Rejected/…). reconciliation phase ค่อย filter `status='Approved'` (เงินเข้าจริง). เก็บ Pending ไว้เพื่อ `ON CONFLICT` หา row เจอตอน status เปลี่ยน. ค่า status ที่เป็นไปได้ = TBD (ยังไม่ enumerate).
  - **Scraping** = extend single Playwright process เดิมของ Apay (login เดียว, ลด auth risk): 23:45 login → 23:59 balance + payment summary (เดิม) → sleep ข้ามเที่ยงคืน → ~00:05 fetch withdraw list. `date0=date1` = **freeze ตอน 23:45** (วันที่เริ่มรัน = วันที่เพิ่งปิด) ไม่ใช่คำนวณตอน fetch. ความเสี่ยง: session ต้องอยู่รอดข้ามเที่ยงคืน + process crash ช่วง sleep = คืนนั้นไม่ได้ list (balance ได้แล้ว) → ต้องมี manual re-run path.
...
- **Account breakdown — Apay report-sourced row** (ADR 0016) — แถว Apay ในตารางดึง ยอดจ่าย/ยอดรับ จาก `report_apay_daily` (scraper>discord priority) **แทน** slip/balance formula: `ยอดจ่ายสุทธิ = withdrawal_amount`, `ยอดรับ = deposit_amount`. column "ยอดจ่ายระบบ" + "Manual" ของแถวนี้ = "—" (gateway ไม่ใช่ slip/manual จึงไม่เข้า 2 column นั้น + ไม่เข้า total ของมัน). มี badge "📊 จากรายงาน" (+ source scraper/discord). `fee_amount` + เวลา scrape ไม่แสดง. balance เปิด/ปิด ยังโชว์จาก `daily_balances` display-only. ไม่มี report row → "—/ไม่มีรายงาน". **เข้า grand total + KPI cards ทั้งหมด**: total row ตาราง (ยอดจ่ายสุทธิ, ยอดรับ), card 2 "ยอดเข้าระบบ(สลิป)", card 3 outflow (relabel — เลิกใช้ "จากสลิป"), card 4 "ส่วนต่าง". card 1 "ยอดเข้าระบบ(เว็บ)" ไม่แตะ. Panel "ข้อมูล Apay Gateway (Cross-check)" เดิม **ลบทิ้ง** (ข้อมูลย้ายเข้าตาราง). เฉพาะ single-project — `projectId='all'` ข้าม merge.
- **Unmatched slip/balance banner** — Banner แจ้งเตือน ที่แสดง count ของ unmatched slips + unmatched daily_balances สำหรับ targetDate. Filter: `(transactions.transfer_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok')::date = targetDate AND matching_status NOT IN ('AUTO_MAPPED','MANUAL_MAPPED')` + `daily_balances.date = targetDate AND matching_status = 'UNMATCHED'`. Link ไป match page พร้อม `?date=targetDate` pre-filled. แสดงเฉพาะตอน count > 0.
...
- **Slip note** (`transactions.slip_note`) — หมายเหตุที่ staff พิมพ์ใน **แอปธนาคารตอนโอน** ติดมาในรูปสลิป (field title ต่างกันต่อธนาคาร: บันทึก/โน๊ต/...). spectre worker OCR เก็บดิบ. = signal หลักของ [[Transaction classification]]. ต่างจาก **Discord caption** (staff พิมพ์ตอนส่งใน Discord). ดู [ADR 0019](docs/adr/0019-slip-note-classification.md).
- **Transaction classification** — AI (spectre worker) จัด Discord slip จาก [[Slip note]] เป็น 2 มิติ: (1) **type** (managed, ADR 0008) + **subtype** (freetext) เช่น `ถอนให้ลูกค้า` / `โอนไบแนน` / `รายจ่าย`→subtype `ค่าเงินเดือน`; (2) **target_project** (cross-project lending) จาก project suffix ใน note. มั่นใจ → auto-assign, ไม่มั่นใจ → type null → admin classify ภายหลัง (ไม่ block — type=metadata phase 1). independent จาก account matching. ดู [ADR 0019](docs/adr/0019-slip-note-classification.md).
  - **Binance** = transaction_type `โอนไบแนน` **ไม่ใช่ project_account** (เป็นปลายทางที่เงินออก ไม่ใช่ master ที่มี balance/reconcile).
  - **Dual-source target** — `target_project` มี 2 candidate: Discord caption (Fuse) + slip_note (AI). ทั้งคู่ระบุ project **ต่างกัน** → conflict → PENDING admin (resolve ใน `SlipReviewDialog`). ฝ่ายเดียวระบุ → ใช้ฝ่ายนั้น. match suffix กับ `projects.code` + `aliases`.
  - **Phase boundary** — phase 1 = classify + store + **display breakdown** (SlipDrawer badge, summary section, AccountBreakdownTable column) ไม่แตะสูตร (ทุก type นับเป็น outflow เหมือนเดิม — balance reconstruction ไม่สนประเภท). phase 2 = ถ้าจะให้ type กระทบ calc → **ต้อง grill ก่อน** (ไม่แตะ = ไม่ต้อง grill).
...
- **Edit/delete policy** — provenance-based. Discord slip (`transactions`) **ลบไม่ได้** — ใช้ REJECT แทน (raw_uploads ต้องอยู่). Manual slip + `daily_balances` ทุก source = hard delete. Edit: AI fields immutable (Discord slip amount override ผ่าน `adjusted_amount`); manual entries แก้ field ใดก็ได้; `daily_balances` มี 2 mode — **full edit** (match page drawer): แก้ `balance_amount`/`note` + re-match + delete ทุก source; **restricted edit** (reconciliation page account breakdown): แก้ได้แค่ `balance_amount`/`note` เท่านั้น — ซ่อน re-match + ซ่อน delete (prop `restrictedEdit` ใน `DailyBalanceDrawer`). Re-match → force `MANUAL_MAPPED` + auto-alias append. RBAC: edit/delete ทั้งหมด = `manage_projects` (owner+admin). Audit columns inline: `last_edited_by/at/note` (note optional), `deleted_at/by/delete_reason` (reason required). UI: actions อยู่ใน detail drawer เท่านั้น ไม่ใช่ inline ในตาราง. Warning dialog เมื่อแก้ `balance_amount` หรือ `transfer_at` (กระทบ reconciliation วันข้างเคียง). Delete row + leave image file (orphan, ops cleanup). ดู [ADR 0011](docs/adr/0011-edit-delete-policy.md).
- **Daily balance one-per-day rule** — 1 row ต่อ (`project_account_id`, `date`) ไม่ว่า source/status ใด. Enforced ที่ DB ด้วย `UNIQUE (date, account_name)` (migration plan). Manual add ที่ชนกับ row เดิม → pre-check ใน `AddAdjustmentDialog` แสดง warning + ปุ่ม "ไปแก้ไขรายการเดิม" เปิด edit drawer.
- **Global date filter** — Date = global state, source of truth = URL query `?date=YYYY-MM-DD`, mirror ไป `sessionStorage` เพื่อ persist cross-page navigation ที่ไม่ส่ง query. Default = **เมื่อวาน** (Bangkok) — scraper รัน 00:30 ดึงเมื่อวาน. ปิด browser → reset เป็นเมื่อวานอีก (ไม่ค้างข้ามวัน). Period (day/week/month/year) = per-page state, persist ใน `localStorage` keyed by page (`fixlo.period.match`, `fixlo.period.dashboard`). Range = anchor-relative: week = ISO Mon-Sun ครอบ date, month/year = calendar month/year ครอบ date. Reconciliation **ไม่มี period state** (ซ่อน selector). Component `<GlobalDateBar />` มี per-page opt-in mount (reconciliation, match, dashboard root) — Header ไม่แตะ (Header = project switcher เท่านั้น). ดู [ADR 0012](docs/adr/0012-global-date-filter.md).
