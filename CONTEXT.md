...
- **created_at timezone caveat** — ⚠️ ต่างจาก [[transfer_at timezone invariant]]. `created_at` ในตาราง `timestamp without time zone` (raw_uploads, transactions, daily_balances, project_accounts, report_* ฯลฯ) เก็บเป็น **Bangkok wall-clock** ไม่ใช่ UTC — เพราะมาจาก `DEFAULT CURRENT_TIMESTAMP` ของ Postgres + DB session timezone = `Asia/Bangkok`. ดังนั้น `transfer_at` (UTC) กับ `created_at` (Bangkok) ใน row เดียวกันต่างกัน 7 ชม. ทั้งที่ column type เหมือนกัน — **กับดัก**: type ไม่บอกว่าอันไหน UTC อันไหน BKK. อ่าน `created_at` เป็น business clock ได้ตรงๆ (ห้าม +7h). ⚠️ frontend `src/lib/db.ts` OID 1114 parser parse-as-UTC → อ่าน `created_at` เพี้ยน +7h (treat Bangkok เป็น UTC) — ระวังตอนโชว์เวลา upload. ข้อยกเว้น: `manual_transactions.created_at` + `transaction_types.created_at` เป็น `timestamptz` (มี tz จริง). Spectre `resolveBalanceDate` (issue #107) พึ่ง convention Bangkok-naive นี้ผ่าน `EXTRACT(EPOCH FROM created_at AT TIME ZONE 'UTC')`. **Standardize เป็น UTC**: migration 044 + [ADR 0015](docs/adr/0015-created-at-utc-standardization.md) — scope app-owned 5 ตาราง (raw_uploads, transactions, daily_balances, project_accounts, projects), `report_*` (scraper) ไม่แตะ. **staged ยังไม่ apply prod** — ต้อง window + deploy spectre PR #7 lockstep (resolveBalanceDate กลับเป็น +7h). หลัง apply: app-owned `created_at` = UTC-naive ตรงกับ transfer_at; `report_*` ยังเป็น Bangkok-naive (inconsistency ที่ยอมรับ).
