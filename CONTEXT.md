...
- **Account breakdown — Daily balance columns** — ตาราง "รายละเอียดการจ่ายแยกตามบัญชี" แสดง 2 columns ก่อน "ยอดรับ": "วันก่อนหน้า" (balance_(D-1)) + "วันที่เลือก" (balance_D). แต่ละ cell แสดง balance amount + status badge (UNMATCHED/PENDING_REVIEW/AUTO_MAPPED/MANUAL_MAPPED). ปุ่ม "ดูรูป" attach ต่อ cell, เปิด modal gallery ของ daily_balance image. ถ้าไม่มี balance ตรงวัน → empty cell (sync ลอจิก "ยอดรับ" formula ที่ต้อง strict-match ทั้งสองวัน).
