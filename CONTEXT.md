...
- **`report_apay_daily`** — daily aggregate ยอดฝาก/ถอนจาก Apay gateway portal (1 row ต่อวันต่อ account). ใช้เป็น cross-check layer ใน reconciliation page — แสดงแยกจาก ยอดเข้าระบบ(สลิป) ไม่เข้าสูตรโดยตรง. UNIQUE `(date, project_account_id)` → ON CONFLICT DO UPDATE. ดู [ADR 0009](docs/adr/0009-apay-gateway-scraper.md).
